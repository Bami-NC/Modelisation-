    ...
    DO WHILE (temps<duree) !Condition d'arrêt
      ...
      DO m=1,nint(moitie)
          ...
          ELSE
            F2=S1*lambda1*(T1(m+1)-T1(m))/dx
            F4=S2*lambda2*(T2(m+1)-T2(m))/dx
            F10=2*S3*lambdaMCP*(Tamb-T3(m))/dx !On considere MCP en contact avec isolant
          END IF
...
SUBROUTINE export()
	...
	OPEN(FICH2,FILE="result_PDC2_amont.txt", ACTION="WRITE", STATUS="UNKNOWN", POSITION="APPEND");
